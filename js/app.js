...
function card(j){return `<article class="job-card show" data-id="${j.id}"><div class="job-header"><div class="job-company-info"><div class="company-logo ${j.color} company-${j.id}" aria-hidden="true">${j.logo}</div><div class="job-info"><h3><a href="#job/${j.id}" class="job-title-link">${esc(j.title)}</a></h3><p class="job-company">${esc(j.company)}</p></div></div><div class="job-actions">${likeButton(j)}<button class="apply-btn" data-apply="${j.id}">สมัครงาน</button></div></div>${tags(j)}<p class="job-description">${esc(j.summary)}</p><div class="job-footer"><span class="job-salary">${salary(j)}</span><time class="job-posted" datetime="${j.date}">ประกาศ ${dateLabel(j.date)}</time></div><a class="details-link" href="#job/${j.id}">ดูรายละเอียดและคุณสมบัติ →</a></article>`;}
function renderJobs(){
